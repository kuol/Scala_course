...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains elements that exist in each set") {
    new TestSets {
      val s: Set = union(s1, s2)
      val t: Set = union(s2, s3)
      val v: Set = intersect(s, t)
  ...
  test("diff contains elements that exist in first set but not the second") {
    new TestSets {
      val s: Set = union(s1, s2)
      val t: Set = diff(s, s1)
  ...
  test("map works properly") {
    new TestSets {
      val s: Set = union(s1, s2)
      val v: Set = map(s, x => 2*x)
      assert(!contains(v, 1), "Map 1")
      assert(contains(v, 2), "Map 2")
      assert(!contains(v, 3), "Map 3")
      assert(contains(v, 4), "Map 4")
    }
  }
}
